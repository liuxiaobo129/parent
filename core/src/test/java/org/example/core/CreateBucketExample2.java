...
public class CreateBucketExample2 {
    public static void main(String[] args) {
        // 1. 配置华为云 OBS 访问密钥
        String endPoint = "obs.cn-southwest-2.myhuaweicloud.com"; // 请修改为实际区域的 OBS 终端节点
        ...
        try {
            // 3. 创建 Bucket 请求
            String bucketName = "bucket-4444"; // 替换为你的 Bucket 名称

            CreateBucketRequest request = new CreateBucketRequest(bucketName);
            request.setBucketStorageClass(StorageClassEnum.STANDARD); // 设置存储类型（默认 STANDARD）
            request.setLocation("cn-southwest-2"); // 设置桶的区域（请填写实际区域）
